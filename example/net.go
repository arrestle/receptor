...
func main() {
    ...
    log := logger.NewReceptorLogger("example")
    logger.SetGlobalLogLevel(logger.DebugLevel)
    log.Info("Starting Receptor example", "field2", "value2")
    ...
    log.Info("Created nodes","node1",n1,"node2",n2)
    ...
    log.Info("Created Backends","b1",b1,"l1",l1.Addr().String())
    go func() {
        ...
        log.Info("Accepted a connection", "local", conn.LocalAddr().String(), "remote", conn.LocalAddr(),"l1",l1.Addr())
        go func() {
            defer conn.Close()
            buf := make([]byte, 1024)
            done := false
    ...
    for {
        ...
        log.Info("Created Backends","n2",n2.NodeID(),"c2",c2.LocalAddr().String())
        break
    }
